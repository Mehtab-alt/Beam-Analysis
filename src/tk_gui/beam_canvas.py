...
class BeamCanvas(tk.Frame):
                ...
                for id_key, data in self.items.items():
                    if data.get("index") == index and data.get("type") == item_type:
                        if item_type == "support":
                            self.canvas.itemconfig(id_key, outline="orange")
                        elif item_type == "load":
                ...
                for id_key, data in self.items.items():
                    if data.get("index") == index and data.get("type") == item_type:
                        ...
                        if item_type == "beam":
                            ...
                            self.canvas.itemconfig(id_key, outline=original_color)
                        elif item_type == "load":
    ...
    def draw_pinned_support(self, x, y):
        ...
        item_id = self.canvas.create_polygon(points, fill="red", outline="black", tags="support")
        return [item_id]
    ...
    def draw_roller_support(self, x, y):
        ...
        rect_id = self.canvas.create_rectangle(x - size, y - size, x + size, y + size,
                                              fill="white", outline="red", width=2, tags="support")
        
        # Small circle to indicate roller
        circle_id = self.canvas.create_oval(x - 5, y + size - 5, x + 5, y + size + 5,
    ...
    def draw_fixed_support(self, x, y, width, height):
        ...
        support_width = 20
        rect_id = self.canvas.create_rectangle(x, y - 30, x + support_width, y + 30,
                                              fill="gray", outline="black", tags="support")
        return [rect_id]
    ...
    def draw_loads(self, width, height):
        ...
        for i, load in enumerate(loads):
            ...
            if load_type == 'point':
                ...
                x = self.padding + pos * self.scale
                item_ids = self.draw_point_load(x, self.beam_y - 30, magnitude, color)
                ...
                item_ids = self.draw_distributed_load(x1, x2, self.beam_y - 30, magnitude, color)
    ...
    def draw_distributed_load(self, x1, x2, y, magnitude, color):
        ...
        length = abs(x2 - x1)
        num_arrows = max(3, int(length / 30))  # At least 3 arrows, more for longer loads
        ...
        for i in range(num_arrows):
            x = x1 + i * step
            arrow_id = self.canvas.create_line(x, y, x, y - arrow_length,
                                              arrow=tk.LAST, fill=color, width=2, tags="load")
            item_ids.append(arrow_id)
        
        # Draw connecting line at top
        line_id = self.canvas.create_line(x1, y - arrow_length, x2, y - arrow_length,
                                         fill=color, width=2, tags="load")
        item_ids.append(line_id)
        
        # Label
        mid_x = (x1 + x2) / 2
        label_id = self.canvas.create_text(mid_x, y - arrow_length - 15, 

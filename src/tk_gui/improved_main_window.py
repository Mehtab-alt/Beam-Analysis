...
from .improved_beam_canvas import ImprovedBeamCanvas
from .improved_toolbox import ImprovedToolbox
from .improved_properties_panel import ImprovedPropertiesPanel
...
class ImprovedMainWindow:
    ...
    def _configure_styles(self):
        ...
        style.configure('Toolbox.TFrame', background='#f0f0f0')
        style.configure('Properties.TFrame', background='#f0f0f0')
        ...
        style.configure('Result.TNotebook', tabposition='n')
        style.configure('Result.TNotebook.Tab', padding=[10, 4])
    ...
    def _create_toolbar(self, parent):
        ...
        file_group = ttk.LabelFrame(toolbar_frame, text="File", padding=2)
        file_group.pack(side=tk.LEFT, padx=(0, 10))
        
        self.open_btn = ttk.Button(file_group, text="Open", 
                                  command=self.open_config_file, style='ToolButton.TButton')
        self.open_btn.pack(side=tk.LEFT, padx=(0, 2))
        
        self.save_btn = ttk.Button(file_group, text="Save", 
                                  command=self.save_config_file, style='ToolButton.TButton')
        self.save_btn.pack(side=tk.LEFT, padx=(0, 2))
        
        # Analysis operations group
        analysis_group = ttk.LabelFrame(toolbar_frame, text="Analysis", padding=2)
        analysis_group.pack(side=tk.LEFT, padx=(0, 10))
        
        self.run_btn = ttk.Button(analysis_group, text="Run Analysis", 
                                 command=self.start_analysis, style='Primary.TButton')
        self.run_btn.pack(side=tk.LEFT, padx=(0, 2))
        
        # View options group
        view_group = ttk.LabelFrame(toolbar_frame, text="View", padding=2)
        view_group.pack(side=tk.LEFT, padx=(0, 10))
        
        # Toolbox toggle
        self.toolbox_visible = tk.BooleanVar(value=True)
        self.toolbox_check = ttk.Checkbutton(view_group, text="Toolbox", 
                                            variable=self.toolbox_visible, 
                                            command=self.toggle_toolbox)
        self.toolbox_check.pack(side=tk.LEFT, padx=(0, 5))
        
        # Properties toggle
        self.properties_visible = tk.BooleanVar(value=True)
        self.properties_check = ttk.Checkbutton(view_group, text="Properties", 
        ...
        self.log_check = ttk.Checkbutton(view_group, text="Logs", 
    ...
    def _create_main_content(self, parent):
        ...
        self.toolbox = ImprovedToolbox(content_frame)
        ...
        self.notebook = ttk.Notebook(canvas_container, style='Result.TNotebook')
        ...
        self.properties_panel = ImprovedPropertiesPanel(content_frame)
        self.properties_panel.set_main_window(self)  # Set reference to main window
    ...
    def _create_canvas(self, parent):
        ...
        self.beam_canvas = ImprovedBeamCanvas(parent)
